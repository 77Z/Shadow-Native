...
int startAXEEditor(AXEProjectEntry project) {
	ShadowWindow axeEditorWindow(1800, 1280, "AXE Editor");
	...
	struct MySequence : public ImSequencer::SequenceInterface {
		...
		virtual void CustomDraw(int index, ImDrawList* draw_list, const ImRect& rc,
			...
			ImCurveEdit::Edit(rampEdit, ImVec2(rc.Max.x - rc.Min.x, rc.Max.y - rc.Min.y),
				137 + index, &clippingRect);
		}

		virtual void CustomDrawCompact(
			int index, ImDrawList* draw_list, const ImRect& rc, const ImRect& clippingRect) {
			rampEdit.mMax = ImVec2(float(mFrameMax), 1.f);
			rampEdit.mMin = ImVec2(float(mFrameMin), 0.f);
