...
// This must be <= 255 since index 0xff is used internally to indice a vertex that doesn't belong to
// a meshlet
...
static void buildTriangleAdjacency(TriangleAdjacency2& adjacency, const unsigned int* indices,
...
static void computeBoundingSphere(float result[4], const float points[][3], size_t count) {
	...
	for (int axis = 0; axis < 3; ++axis) {
		...
		float d2 = (p2[0] - p1[0]) * (p2[0] - p1[0]) + (p2[1] - p1[1]) * (p2[1] - p1[1])
			+ (p2[2] - p1[2]) * (p2[2] - p1[2]);
...
static float getMeshletScore(
	float distance2, float spread, float cone_weight, float expected_radius) {
...
static float computeTriangleCones(Cone* triangles, const unsigned int* indices, size_t index_count,
...
static bool appendMeshlet(meshopt_Meshlet& meshlet, unsigned int a, unsigned int b, unsigned int c,
	unsigned char* used, meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices,
	unsigned char* meshlet_triangles, size_t meshlet_offset, size_t max_vertices,
...
static unsigned int getNeighborTriangle(const meshopt_Meshlet& meshlet, const Cone* meshlet_cone,
	unsigned int* meshlet_vertices, const unsigned int* indices,
	const TriangleAdjacency2& adjacency, const Cone* triangles, const unsigned int* live_triangles,
	const unsigned char* used, float meshlet_expected_radius, float cone_weight,
	unsigned int* out_extra) {
	...
	for (size_t i = 0; i < meshlet.vertex_count; ++i) {
		...
		for (size_t j = 0; j < neighbors_size; ++j) {
			...
			if (extra != 0) {
				// artificially increase the priority of dangling triangles as they're expensive to
				// add to new meshlets
			...
			// since topology-based priority is always more important than the score, we can skip
			...
			if (meshlet_cone) {
				...
				float spread = tri_cone.nx * meshlet_cone->nx + tri_cone.ny * meshlet_cone->ny
					+ tri_cone.nz * meshlet_cone->nz;

				score = getMeshletScore(distance2, spread, cone_weight, meshlet_expected_radius);
			} else {
				// each live_triangles entry is >= 1 since it includes the current triangle we're
				// processing
...
static size_t kdtreePartition(unsigned int* indices, size_t count, const float* points,
	size_t stride, unsigned int axis, float pivot) {
	size_t m = 0;
...
static size_t kdtreeBuildLeaf(
	size_t offset, KDNode* nodes, size_t node_count, unsigned int* indices, size_t count) {
...
static size_t kdtreeBuild(size_t offset, KDNode* nodes, size_t node_count, const float* points,
	...
	size_t next_offset
		= kdtreeBuild(offset + 1, nodes, node_count, points, stride, indices, middle, leaf_size);

	// distance to the right subtree is represented explicitly
	result.children = unsigned(next_offset - offset - 1);

	return kdtreeBuild(next_offset, nodes, node_count, points, stride, indices + middle,
		count - middle, leaf_size);
}
...
static void kdtreeNearest(KDNode* nodes, unsigned int root, const float* points, size_t stride,
	...
	if (node.axis == 3) {
		...
		kdtreeNearest(
			nodes, root + 1 + first, points, stride, emitted_flags, position, result, limit);

		// only process the other node if it can have a match based on closest distance so far
		if (fabsf(delta) <= limit)
			kdtreeNearest(
				nodes, root + 1 + second, points, stride, emitted_flags, position, result, limit);
	}
}
...
size_t meshopt_buildMeshletsBound(size_t index_count, size_t max_vertices, size_t max_triangles) {
	...
	assert(max_triangles % 4
		== 0); // ensures the caller will compute output space properly as index data is 4b aligned

	(void)kMeshletMaxVertices;
	(void)kMeshletMaxTriangles;

	// meshlet construction is limited by max vertices and max triangles per meshlet
	// the worst case is that the input is an unindexed stream since this equally stresses both
	// limits note that we assume that in the worst case, we leave 2 vertices unpacked in each
	// meshlet - if we have space for 3 we can pack any triangle
	...
	return meshlet_limit_vertices > meshlet_limit_triangles ? meshlet_limit_vertices
...
size_t meshopt_buildMeshlets(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices,
	...
	float mesh_area = computeTriangleCones(
		triangles, indices, index_count, vertex_positions, vertex_count, vertex_positions_stride);
	...
	kdtreeBuild(0, nodes, face_count * 2, &triangles[0].px, sizeof(Cone) / sizeof(float), kdindices,
		face_count, /* leaf_size= */ 8);
	...
	for (;;) {
		...
		unsigned int best_triangle
			= getNeighborTriangle(meshlet, &meshlet_cone, meshlet_vertices, indices, adjacency,
				triangles, live_triangles, used, meshlet_expected_radius, cone_weight, &best_extra);
		...
		if (best_triangle != ~0u
			&& (meshlet.vertex_count + best_extra > max_vertices
				|| meshlet.triangle_count >= max_triangles)) {
			best_triangle = getNeighborTriangle(meshlet, NULL, meshlet_vertices, indices, adjacency,
				triangles, live_triangles, used, meshlet_expected_radius, 0.f, NULL);
		}

		// when we run out of neighboring triangles we need to switch to spatial search; we
		// currently just pick the closest triangle irrespective of connectivity
		if (best_triangle == ~0u) {
		...
		unsigned int a = indices[best_triangle * 3 + 0], b = indices[best_triangle * 3 + 1],
					 c = indices[best_triangle * 3 + 2];
		assert(a < vertex_count && b < vertex_count && c < vertex_count);

		// add meshlet to the output; when the current meshlet is full we reset the accumulated
		// bounds
		if (appendMeshlet(meshlet, a, b, c, used, meshlets, meshlet_vertices, meshlet_triangles,
...
size_t meshopt_buildMeshletsScan(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices,
	unsigned char* meshlet_triangles, const unsigned int* indices, size_t index_count,
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		meshlet_offset += appendMeshlet(meshlet, a, b, c, used, meshlets, meshlet_vertices,
...
meshopt_Bounds meshopt_computeClusterBounds(const unsigned int* indices, size_t index_count,
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		// record triangle normals & corners for future use; normal and corner 0 define a plane
		// equation
	...
	// treating triangle normals as points, find the bounding sphere - the sphere center determines
	// the optimal cone axis
	float nsphere[4] = {};
	...
	// fill bounding sphere info; note that below we can return bounds without cone information for
	// degenerate cones
	...
	// note that if mindp is positive but close to 0, the triangle intersection code below gets less
	// stable we arbitrarily decide that if a normal cone is ~168 degrees wide or more, the cone
	// isn't useful
	if (mindp <= 0.1f) {
	...
	// we need to find the point on center-t*axis ray that lies in negative half-space of all
	// triangles
	for (size_t i = 0; i < triangles; ++i) {
	...
	// note: this axis is the axis of the normal cone, but our test for perspective camera
	// effectively negates the axis
	...
	// for the 8-bit test to be conservative, we need to adjust the cutoff by measuring the max.
	// error
	...
	int cone_cutoff_s8
		= int(127 * (bounds.cone_cutoff + cone_axis_s8_e0 + cone_axis_s8_e1 + cone_axis_s8_e2) + 1);
...
meshopt_Bounds meshopt_computeMeshletBounds(const unsigned int* meshlet_vertices,
	...
	return meshopt_computeClusterBounds(
		indices, triangle_count * 3, vertex_positions, vertex_count, vertex_positions_stride);
}
