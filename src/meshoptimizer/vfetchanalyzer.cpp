...
meshopt_VertexFetchStatistics meshopt_analyzeVertexFetch(
	...
	// simple direct mapped cache; on typical mesh data this is close to 4-way cache, and this model
	...
	result.overfetch = unique_vertex_count == 0
		? 0
		: float(result.bytes_fetched) / float(unique_vertex_count * vertex_size);
