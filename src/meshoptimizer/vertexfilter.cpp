...
// The SIMD implementation requires SSE2, which can be enabled unconditionally through compiler
// settings
#if defined(__SSE2__)
#define SIMD_SSE
#endif

// MSVC supports compiling SSE2 code regardless of compile options; we assume all 32-bit CPUs
// support SSE2
...
template <typename T> static void decodeFilterOct(T* data, size_t count) {
	...
	for (size_t i = 0; i < count; ++i) {
		// convert x and y to floats and reconstruct z; this assumes zf encodes 1.f at the same bit
		// count
...
static void decodeFilterOctSimd(signed char* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		__m128 z = _mm_sub_ps(
			_mm_cvtepi32_ps(zf), _mm_add_ps(_mm_andnot_ps(sign, x), _mm_andnot_ps(sign, y)));
...
static void decodeFilterOctSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		__m128i xzr
			= _mm_or_si128(_mm_and_si128(xr, _mm_set1_epi32(0xffff)), _mm_slli_epi32(zr, 16));
		...
		// patch in .w
		res_0 = _mm_or_si128(
			res_0, _mm_and_si128(_mm_castps_si128(n4_0), _mm_set1_epi64x(0xffff000000000000)));
		res_1 = _mm_or_si128(
			res_1, _mm_and_si128(_mm_castps_si128(n4_1), _mm_set1_epi64x(0xffff000000000000)));
...
static void decodeFilterQuatSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		// reconstruct w as a square root; we clamp to 0.f to avoid NaN due to precision errors
		__m128 ww = _mm_sub_ps(_mm_set1_ps(1.f),
			_mm_add_ps(_mm_mul_ps(x, x), _mm_add_ps(_mm_mul_ps(y, y), _mm_mul_ps(z, z))));
		...
		__m128i xzr
			= _mm_or_si128(_mm_and_si128(xr, _mm_set1_epi32(0xffff)), _mm_slli_epi32(zr, 16));
		__m128i wyr
			= _mm_or_si128(_mm_and_si128(wr, _mm_set1_epi32(0xffff)), _mm_slli_epi32(yr, 16));
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		x = vaddq_f32(x,
			vreinterpretq_f32_s32(
				veorq_s32(vreinterpretq_s32_f32(t), vandq_s32(vreinterpretq_s32_f32(x), sign))));
		y = vaddq_f32(y,
			vreinterpretq_f32_s32(
				veorq_s32(vreinterpretq_s32_f32(t), vandq_s32(vreinterpretq_s32_f32(y), sign))));
		...
		// fast rounded signed float->int: addition triggers renormalization after which mantissa
		// stores the integer value note: the result is offset by 0x4B40_0000, but we only need the
		// low 16 bits so we can omit the subtraction
...
static void decodeFilterOctSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		int32x4_t res_0 = vreinterpretq_s32_s16(
			vzipq_s16(vreinterpretq_s16_s32(xzr), vreinterpretq_s16_s32(y0r)).val[0]);
		int32x4_t res_1 = vreinterpretq_s32_s16(
			vzipq_s16(vreinterpretq_s16_s32(xzr), vreinterpretq_s16_s32(y0r)).val[1]);
...
static void decodeFilterQuatSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		// reconstruct w as a square root; we clamp to 0.f to avoid NaN due to precision errors
		float32x4_t ww = vsubq_f32(vdupq_n_f32(1.f),
		...
		// pack x/y/z/w using 16-bit unpacks; we pack wxyz by default (for qc=0)
		int32x4_t res_0 = vreinterpretq_s32_s16(
			vzipq_s16(vreinterpretq_s16_s32(wyr), vreinterpretq_s16_s32(xzr)).val[0]);
		int32x4_t res_1 = vreinterpretq_s32_s16(
			vzipq_s16(vreinterpretq_s16_s32(wyr), vreinterpretq_s16_s32(xzr)).val[1]);

		// rotate and store
		uint64_t* out = (uint64_t*)&data[i * 4];

		out[0] = rotateleft64(
			vgetq_lane_u64(vreinterpretq_u64_s32(res_0), 0), vgetq_lane_s32(cf, 0) << 4);
		out[1] = rotateleft64(
			vgetq_lane_u64(vreinterpretq_u64_s32(res_0), 1), vgetq_lane_s32(cf, 1) << 4);
		out[2] = rotateleft64(
			vgetq_lane_u64(vreinterpretq_u64_s32(res_1), 0), vgetq_lane_s32(cf, 2) << 4);
		out[3] = rotateleft64(
			vgetq_lane_u64(vreinterpretq_u64_s32(res_1), 1), vgetq_lane_s32(cf, 3) << 4);
	}
}
...
static void decodeFilterOctSimd(signed char* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		v128_t z = wasm_f32x4_sub(
			wasm_f32x4_convert_i32x4(zf), wasm_f32x4_add(wasm_f32x4_abs(x), wasm_f32x4_abs(y)));
		...
		v128_t ll = wasm_f32x4_add(
			wasm_f32x4_mul(x, x), wasm_f32x4_add(wasm_f32x4_mul(y, y), wasm_f32x4_mul(z, z)));
		v128_t s = wasm_f32x4_div(wasm_f32x4_splat(127.f), wasm_f32x4_sqrt(ll));

		// fast rounded signed float->int: addition triggers renormalization after which mantissa
		// stores the integer value note: the result is offset by 0x4B40_0000, but we only need the
		// low 8 bits so we can omit the subtraction
...
static void decodeFilterOctSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		// mix x/z and y/0 to make 16-bit unpack easier
		v128_t xzr
			= wasm_v128_or(wasm_v128_and(xr, wasm_i32x4_splat(0xffff)), wasm_i32x4_shl(zr, 16));
...
static void decodeFilterQuatSimd(short* data, size_t count) {
	...
	for (size_t i = 0; i < count; i += 4) {
		...
		v128_t xzr
			= wasm_v128_or(wasm_v128_and(xr, wasm_i32x4_splat(0xffff)), wasm_i32x4_shl(zr, 16));
		v128_t wyr
			= wasm_v128_or(wasm_v128_and(wr, wasm_i32x4_splat(0xffff)), wasm_i32x4_shl(yr, 16));
...
inline int optlog2(float v) {
	...
	u.f = v;
	// +1 accounts for implicit 1. in mantissa; denormalized numbers will end up clamped to min_exp
	// by calling code
...
void meshopt_encodeFilterQuat(
	void* destination_, size_t count, size_t stride, int bits, const float* data) {
	assert(stride == 8);
...
void meshopt_encodeFilterExp(void* destination_, size_t count, size_t stride, int bits,
	const float* data, enum meshopt_EncodeExpMode mode) {
	using namespace meshopt;
	...
	for (size_t i = 0; i < count; ++i) {
		...
		for (size_t j = 0; j < stride_float; ++j) {
			...
			// note that we additionally scale the mantissa to make it a K-bit signed integer (K-1
			// bits for magnitude)
			exp -= (bits - 1);
