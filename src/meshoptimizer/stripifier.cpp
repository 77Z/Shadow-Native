...
// Francine Evans, Steven Skiena and Amitabh Varshney. Optimizing Triangle Strips for Fast
// Rendering. 1996
namespace meshopt {
...
static int findStripNext(
	const unsigned int buffer[][3], unsigned int buffer_size, unsigned int e0, unsigned int e1) {
	for (size_t i = 0; i < buffer_size; ++i) {
...
size_t meshopt_stripify(unsigned int* destination, const unsigned int* indices, size_t index_count,
	size_t vertex_count, unsigned int restart_index) {
	...
	while (buffer_size > 0 || index_offset < index_count) {
		...
		if (next >= 0) {
			...
			int cont
				= findStripNext(buffer, buffer_size, parity ? strip[1] : v, parity ? v : strip[1]);
			int swap = cont < 0
				? findStripNext(buffer, buffer_size, parity ? v : strip[0], parity ? strip[0] : v)
				: -1;
			...
			// we need to pre-rotate the triangle so that we will find a match in the existing
			...
			// in some cases we can have several matching edges; since we can pick any edge, we pick
			// the one with the smallest triangle index in the buffer. this reduces the effect of
			// stripification on ACMR and additionally - for unclear reasons - slightly improves the
			// stripification efficiency
			int mine = INT_MAX;
...
size_t meshopt_unstripify(unsigned int* destination, const unsigned int* indices,
	...
	for (size_t i = 0; i < index_count; ++i) {
		if (restart_index && indices[i] == restart_index) {
			...
			// although we use restart indices, strip swaps still produce degenerate triangles, so
			// skip them
			if (a != b && a != c && b != c) {
