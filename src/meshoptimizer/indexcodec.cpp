...
static int getEdgeFifo(
	EdgeFifo fifo, unsigned int a, unsigned int b, unsigned int c, size_t offset) {
	for (int i = 0; i < 16; ++i) {
...
static void writeTriangle(void* destination, size_t offset, size_t index_size, unsigned int a,
...
size_t meshopt_encodeIndexBuffer(
	...
	// use static encoding table; it's possible to pack the result and then build an optimal table
	// and repack for now we keep it simple and use the table that has been generated based on
	// symbol frequency on a training mesh set
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		int fer
			= getEdgeFifo(edgefifo, indices[i + 0], indices[i + 1], indices[i + 2], edgefifooffset);
		...
		if (fer >= 0 && (fer >> 2) < 15) {
			...
			*code++ = (unsigned char)((fe << 4) | fec);
			...
			// we only need to push third vertex since first two are likely already in the vertex
			// fifo
			if (fec == 0 || fec >= fecmax)
			...
			if (a == 0 && b == 1 && c == 2 && next > 0 && version >= 1) {
				...
				// reset vertex fifo to make sure we don't accidentally reference vertices from that
				// in the future this makes sure next continues to get incremented instead of being
				// stuck
				memset(vertexfifo, -1, sizeof(vertexfifo));
			}
			...
			// after rotation, a is almost always equal to next, so we don't waste bits on FIFO
			// encoding for a
			...
			// all three edges aren't in the fifo; pushing all of them is important so that we can
			// match them for later triangles
	...
	// add codeaux encoding table to the end of the stream; this is used for decoding codeaux *and*
	// as padding we need padding for decoding to be able to assume that each triangle is encoded as
	// <= 16 bytes of extra data this is enough space for aux byte + 5 bytes per varint index which
	// is the absolute worst case for any input
	...
	// since we encode restarts as codeaux without a table reference, we need to make sure 00 is
	// encoded as a table reference
...
int meshopt_decodeIndexBuffer(void* destination, size_t index_count, size_t index_size,
	...
	// since we store 16-byte codeaux table at the end, triangle data has to begin before
	// data_safe_end
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		if (codetri < 0xf0) {
			...
			// inside this if we try to stay branchless (by using cmov/etc.) since these aren't
			// predictable
			if (fec < fecmax) {
				// fifo reads are wrapped around 16 entry buffer
				...
				// push vertex/edge fifo must match the encoding step *exactly* otherwise the data
			...
			if (codetri < 0xfe) {
				...
				// fifo reads are wrapped around 16 entry buffer
				// also note that we increment next for all three vertices before decoding indices -
				// this matches encoder behavior
				unsigned int a = next++;
...
size_t meshopt_encodeIndexSequence(
	...
	for (size_t i = 0; i < index_count; ++i) {
		...
		// we want the encoded delta to fit into one byte (7 bits), but 2 bits are used for sign and
		// baseline index for now we immediately switch the baseline when delta grows too large -
		// this can be adjusted arbitrarily
		...
		// note: low bit encodes the index of the last baseline which will be used for
		// reconstruction
...
int meshopt_decodeIndexSequence(void* destination, size_t index_count, size_t index_size,
	const unsigned char* buffer, size_t buffer_size) {
	using namespace meshopt;
