...
// The SIMD implementation requires SSSE3, which can be enabled unconditionally through compiler
// settings
#if defined(__AVX__) || defined(__SSSE3__)
#define SIMD_SSE
#endif

// An experimental implementation using AVX512 instructions; it's only enabled when AVX512 is
// enabled through compiler settings
...
// MSVC supports compiling SSSE3 code regardless of compile options; we use a cpuid-based scalar
// fallback
...
// GCC 4.9+ and clang 3.8+ support targeting SIMD ISA from individual functions; we use a
...
// When targeting AArch64/x64, optimize for latency to allow decoding of individual 16-byte groups
// to overlap We don't do this for 32-bit systems because we need 64-bit math for this and this will
// hurt in-order CPUs
...
#define wasmx_unpacklo_v8x16(a, b)                                                                 \
	wasm_i8x16_shuffle(a, b, 0, 16, 1, 17, 2, 18, 3, 19, 4, 20, 5, 21, 6, 22, 7, 23)
#define wasmx_unpackhi_v8x16(a, b)                                                                 \
	wasm_i8x16_shuffle(a, b, 8, 24, 9, 25, 10, 26, 11, 27, 12, 28, 13, 29, 14, 30, 15, 31)
...
inline unsigned char zigzag8(unsigned char v) { return ((signed char)(v) >> 7) ^ (v << 1); }
...
static unsigned char* encodeVertexBlock(unsigned char* data, unsigned char* data_end,
	const unsigned char* vertex_data, size_t vertex_count, size_t vertex_size,
	...
	for (size_t k = 0; k < vertex_size; ++k) {
		...
		data = encodeBytes(
			data, data_end, buffer, (vertex_count + kByteGroupSize - 1) & ~(kByteGroupSize - 1));
		if (!data)
			return 0;
	}
	...
	|| (!defined(SIMD_SSE) && !defined(SIMD_NEON) && !defined(SIMD_AVX) && !defined(SIMD_WASM))
static const unsigned char* decodeBytesGroup(
	const unsigned char* data, unsigned char* buffer, int bitslog2) {
#define READ() byte = *data++
#define NEXT(bits)                                                                                 \
	enc = byte >> (8 - bits), byte <<= bits, encv = *data_var,                                     \
	*buffer++ = (enc == (1 << bits) - 1) ? encv : enc, data_var += (enc == (1 << bits) - 1)
...
static const unsigned char* decodeBytes(const unsigned char* data, const unsigned char* data_end,
...
static const unsigned char* decodeVertexBlock(const unsigned char* data,
	const unsigned char* data_end, unsigned char* vertex_data, size_t vertex_count,
...
#ifdef __wasm__
__attribute__((
	cold)) // this saves 500 bytes in the output binary - we don't need to vectorize this loop!
#endif
static bool
...
static __m128i decodeShuffleMask(unsigned char mask0, unsigned char mask1) {
	__m128i sm0
		= _mm_loadl_epi64(reinterpret_cast<const __m128i*>(&kDecodeBytesGroupShuffle[mask0]));
	__m128i sm1
		= _mm_loadl_epi64(reinterpret_cast<const __m128i*>(&kDecodeBytesGroupShuffle[mask1]));
...
SIMD_TARGET
static const unsigned char* decodeBytesGroupSimd(
	const unsigned char* data, unsigned char* buffer, int bitslog2) {
	switch (bitslog2) {
	case 0: {
	...
	case 1: {
		...
		// adds all 1-bit nibbles together; the sum fits in 4 bits because datacnt=16 would have
		// used mode 3
...
#ifdef SIMD_NEON
static uint8x16_t shuffleBytes(
	unsigned char mask0, unsigned char mask1, uint8x8_t rest0, uint8x8_t rest1) {
...
static void transpose8(uint8x16_t& x0, uint8x16_t& x1, uint8x16_t& x2, uint8x16_t& x3) {
	...
	uint16x8x2_t x01
		= vzipq_u16(vreinterpretq_u16_u8(t01.val[0]), vreinterpretq_u16_u8(t23.val[0]));
	uint16x8x2_t x23
		= vzipq_u16(vreinterpretq_u16_u8(t01.val[1]), vreinterpretq_u16_u8(t23.val[1]));
...
static const unsigned char* decodeBytesSimd(const unsigned char* data,
	const unsigned char* data_end, unsigned char* buffer, size_t buffer_size) {
	assert(buffer_size % kByteGroupSize == 0);
...
SIMD_TARGET
static const unsigned char* decodeVertexBlockSimd(const unsigned char* data,
	const unsigned char* data_end, unsigned char* vertex_data, size_t vertex_count,
	size_t vertex_size, unsigned char last_vertex[256]) {
	assert(vertex_count > 0 && vertex_count <= kVertexBlockMaxSize);
	...
	for (size_t k = 0; k < vertex_size; k += 4) {
		for (size_t j = 0; j < 4; ++j) {
			data = decodeBytesSimd(
				data, data_end, buffer + j * vertex_count_aligned, vertex_count_aligned);
			if (!data)
				return 0;
		}
...
#define LOAD(i)                                                                                    \
	__m128i r##i                                                                                   \
		= _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffer + j + i * vertex_count_aligned))
#define GRP4(i)                                                                                    \
...
#define PREP()                                                                                     \
	uint8x8_t pi = vreinterpret_u8_u32(                                                            \
		vld1_lane_u32(reinterpret_cast<uint32_t*>(last_vertex + k), vdup_n_u32(0), 0))
...
#define SAVE(i)                                                                                    \
	vst1_lane_u32(reinterpret_cast<uint32_t*>(savep), vreinterpret_u32_u8(t##i), 0),               \
		savep += vertex_size
#endif
...
#define SAVE(i)                                                                                    \
	*reinterpret_cast<int*>(savep) = wasm_i32x4_extract_lane(t##i, 0), savep += vertex_size
#endif
...
size_t meshopt_encodeVertexBuffer(unsigned char* buffer, size_t buffer_size, const void* vertices,
	...
	*data++ = (unsigned char)(kVertexHeader | version);
	...
	while (vertex_offset < vertex_count) {
		...
		data = encodeVertexBlock(data, data_end, vertex_data + vertex_offset * vertex_size,
	...
	// write first vertex to the end of the stream and pad it to 32 bytes; this is important to
	// simplify bounds checks in decoder
...
size_t meshopt_encodeVertexBufferBound(size_t vertex_count, size_t vertex_size) {
	...
	return 1
		+ vertex_block_count * vertex_size * (vertex_block_header_size + vertex_block_data_size)
		+ tail_size;
}
...
int meshopt_decodeVertexBuffer(void* destination, size_t vertex_count, size_t vertex_size,
	...
	const unsigned char* (*decode)(const unsigned char*, const unsigned char*, unsigned char*,
		size_t, size_t, unsigned char[256])
		= 0;
	...
	while (vertex_offset < vertex_count) {
		...
		data = decode(data, data_end, vertex_data + vertex_offset * vertex_size, block_size,
			vertex_size, last_vertex);
		if (!data)
			return -2;
