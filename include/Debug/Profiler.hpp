#ifndef SHADOW_NATIVE_DEBUG_PROFILER_HPP
#define SHADOW_NATIVE_DEBUG_PROFILER_HPP
...
#ifdef CONFIG_ENABLE_SHADOW_PROFILER
...
struct interval {
	interval() { }
	interval(const type::id id)
		: id(id)
		, start(RAPID_PROFILE_NOW()) { }
	type::time duration() const { return type::duration(stop - start).count(); }
	type::id id;
...
template <class T> class chunker {
public:
	//------------------------------------------------------------------------//
	...
	~chunker() {
		for (typename std::list<std::vector<T>*>::iterator it = chunks_.begin();
			 it != chunks_.end(); it++)
			delete *it;
		chunks_.clear();
	}
	...
	std::list<std::vector<T>*>& chunks() { return chunks_; }
...
private:
	...
	static void exit() { log(); }

	//------------------------------------------------------------------------//

	static void signal(int signum) { ::exit(signum); }
	...
	static void log() {
		...
		for (typename std::vector<interval::tag<N>>::iterator it = tags.begin(); it != tags.end();
			 it++) {
			file << (it - tags.begin()) << ',' << it->name << ',' << it->file << ',' << it->line
				 << std::endl;
		}
		file.close();
		...
		for (std::list<std::vector<interval>*>::iterator it = chunks.begin(); it != chunks.end();
			 it++) {
			for (std::vector<interval>::iterator vit = (*it)->begin(); vit != (*it)->end(); vit++) {
				file << vit->id << ',' << relative(vit->start) * 1e6 << ','
					 << relative(vit->stop) * 1e6 << ',' << relative(vit->stop, vit->start) * 1e6
					 << std::endl;
			}
		}
		file.close();
...
#endif /* SHADOW_NATIVE_DEBUG_PROFILER_HPP */
